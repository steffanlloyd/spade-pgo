 ...
 */
std::vector<std::pair<int, int>> NearKFDetector::getNearKFCandidates(const std::vector<Eigen::Isometry3d>& kf_updated)
{
    ...
    while (this->process_kf_id_ < static_cast<int>(kf_updated.size()) - 1) {
        ...
        for (int j = 0; j < static_cast<int>(kf_updated.size()) - this->params_->near_kf.min_kf_seperation; ++j) {
            ...
            if (distance2 < std::pow(this->params_->near_kf.distance_threshold, 2)) {
                ...
                ROS_INFO("Added candidate loop closure pair %d and %d, distance %.3f m < %.3f m", j, this->process_kf_id_, std::sqrt(distance2), this->params_->near_kf.distance_threshold);
            }
        }
    }
