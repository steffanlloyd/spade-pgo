...
inline void saveTransformsToFile(
    ...
    const std::vector<uint8_t>& session_ids,
    const std::string& filename)
{
    std::fstream stream(filename.c_str(), std::fstream::out);

    // Header
    stream << "# keyframe_id session_id r00 r01 r02 t0 r10 r11 r12 t1 r20 r21 r22 t2" << std::endl;

    for (size_t i = 0; i < transforms.size(); ++i) {
        const auto& Ti = transforms[i];
        uint8_t session_id = (i < session_ids.size()) ? session_ids[i] : 0;

        stream << i << " " << static_cast<int>(session_id) << " "
