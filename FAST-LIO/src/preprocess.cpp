...
void Preprocess::process_(const sensor_msgs::PointCloud2::ConstPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{  
  avia_handler(msg);
  *pcl_out = pl_surf;
}
...
void Preprocess::avia_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  double t1 = omp_get_wtime();
  int plsize = msg->width;
  // cout<<"plsie: "<<plsize<<endl;
  ...
  sensor_msgs::PointCloud2ConstIterator<uint16_t> iter_tag(*msg, "tag");
  sensor_msgs::PointCloud2ConstIterator<uint16_t> iter_line(*msg, "line");
  sensor_msgs::PointCloud2ConstIterator<float> iter_timestamp(*msg, "timestamp");

  pl_full[0].x = *iter_x;
  pl_full[0].y = *iter_y;
  pl_full[0].z = *iter_z;
  pl_full[0].intensity = *iter_intensity;
  pl_full[0].curvature = *iter_timestamp; //use curvature as time of each laser points
    ...
    ++iter_line;
    ++iter_timestamp;
    for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z, ++iter_intensity,++iter_tag, ++iter_line, ++iter_timestamp, ++i)
    { 
      if((*iter_line < N_SCANS) && ((*iter_tag & 0x30) == 0x10 || (*iter_tag & 0x30) == 0x00))   
      {
        valid_num ++;
        if (valid_num % point_filter_num == 0)
        {
          ...
          pl_full[i].y = *iter_y;
          pl_full[i].z = *iter_z;
          pl_full[i].intensity = *iter_intensity;
          pl_full[i].curvature = *iter_timestamp - pl_full[0].curvature; //use curvature as time of each laser points

          cout << pl_full[i].curvature << endl;
